...
public class nameInput : MonoBehaviour
{
    ...
    public void SaveAndClosePopup()
    {
        ...
        if (IsValidName(firstName) && IsValidName(secondName))
        {
            ...
            PlayerPrefs.SetString("PlayerName", playerName); // ¼öÁ¤µÈ ºÎºÐ
            PlayerPrefs.SetString("FirstName", firstName); // ¼öÁ¤µÈ ºÎºÐ
            PlayerPrefs.SetString("SecondName", secondName); // ¼öÁ¤µÈ ºÎºÐ
            PlayerPrefs.Save();
            Debug.Log("Player name saved: " + playerName);
            Debug.Log("FirstName saved: " + firstName); // Ãß°¡µÈ µð¹ö±× ·Î±×
            Debug.Log("SecondName saved: " + secondName); // Ãß°¡µÈ µð¹ö±× ·Î±×
